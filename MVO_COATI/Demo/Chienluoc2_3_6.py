...
# Tính toán fitness cho tất cả các cá thể
...
# Lựa chọn bằng cách quay bánh xe (roulette wheel) trong MVO
...
# Thuật toán lai COA-MVO với adaptive switching, blended update và adaptive combination mechanism
def hybrid_coa_mvo(pop_size, bounds, max_iter, n, stagnation_limit=100):
    # Khởi tạo dân số
    ...
    stagnation_counter = 0  # Đếm số vòng lặp không cải thiện fitness
    mvo_ratio = 0.5  # Tỷ lệ giữa MVO và COA (Adaptive Combination)

    for iter in range(max_iter):
        WEP = WEPmin + (iter / max_iter) * (WEPmax - WEPmin)
        TDR = 1 - (iter / max_iter)

        # Adaptive switching: Nếu không cải thiện sau nhiều vòng lặp, điều chỉnh tỷ lệ MVO/COA
        if stagnation_counter >= stagnation_limit:
            print(f"Adjusting strategy after {iter} iterations due to stagnation.")
            mvo_ratio = max(0.2, mvo_ratio - 0.1)  # Giảm tỷ lệ MVO, tăng tỷ lệ COA để thoát cục bộ
            stagnation_counter = 0  # Reset đếm sau khi điều chỉnh chiến lược

        # Blended update: Tỷ lệ giữa MVO và COA được điều chỉnh dựa trên mvo_ratio
        num_mvo = int(mvo_ratio * len(population))
        num_coa = len(population) - num_mvo

        # Cập nhật một phần quần thể theo MVO
        population[:num_mvo] = update_population_mvo(population[:num_mvo], fitness[:num_mvo], best_solution, WEP, bounds, TDR)
        
        # Cập nhật phần còn lại của quần thể theo COA
        population[num_mvo:] = update_population_coa(population[num_mvo:], best_solution, bounds)
        ...
        if current_best_fitness < best_fitness:
            ...
            stagnation_counter = 0  # Reset khi có sự cải thiện
        else:
...
# Chạy thuật toán
best_solution, best_fitness = hybrid_coa_mvo(pop_size, bounds, max_iter, n)

print("Best solution found:")
for idx, value in enumerate(best_solution):
    print(f"  Variable {idx + 1}: {value:.7f}")
print(f"With fitness: {best_fitness:.7f}")
