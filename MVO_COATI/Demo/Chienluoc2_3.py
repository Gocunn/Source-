...
# Thuật toán lai COA-MVO với adaptive switching và blended update
def hybrid_coa_mvo(pop_size, bounds, max_iter, n, stagnation_limit=100, switch_iter=200):
    # Khởi tạo dân số
    ...
    for iter in range(max_iter):
        ...
        # Adaptive switching: Nếu không cải thiện sau nhiều vòng lặp, đổi chiến lược
        if stagnation_counter >= stagnation_limit:
            print(f"Switching strategy after {iter} iterations due to stagnation.")
            stagnation_counter = 0  # Reset đếm sau khi đổi chiến lược
        
        # Blended update: Chia nửa quần thể để cập nhật theo MVO hoặc COA
        half_pop = len(population) // 2

        # Cập nhật nửa quần thể theo MVO
        population[:half_pop] = update_population_mvo(population[:half_pop], fitness[:half_pop], best_solution, WEP, bounds, TDR)
        
        # Cập nhật nửa quần thể theo COA
        population[half_pop:] = update_population_coa(population[half_pop:], best_solution, bounds)
        ...
        # Điều kiện dừng khi đến số vòng lặp tối đa
        if iter == max_iter - 1:
            print(f"Stopping after {max_iter} iterations.")
